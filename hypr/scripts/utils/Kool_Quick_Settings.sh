...
UserScripts="$HOME/.config/hypr/scripts/user"
...
toggle_rainbow_borders() {
    ...
    local refresh_script="$scriptsDir/display/Refresh.sh"
    local status=""
...
main() {
    ...
    case "$choice" in
        ...
        "Set SDDM Wallpaper") $scriptsDir/utils/sddm_wallpaper.sh --normal ;;
        "Choose Kitty Terminal Theme") $scriptsDir/theme/Kitty_themes.sh ;;
        "Configure Monitors (nwg-displays)")
		...
		"QT Apps Settings (qt5ct)")
            ...
            qt5ct ;;
        "Choose Hyprland Animations") $scriptsDir/theme/Animations.sh ;;
        "Choose Monitor Profiles") $scriptsDir/display/MonitorProfiles.sh ;;
        "Choose Rofi Themes") $scriptsDir/rofi/RofiThemeSelector.sh ;;
        "Search for Keybinds") $scriptsDir/input/KeyBinds.sh ;;
        "Toggle Game Mode") $scriptsDir/system/GameMode.sh ;;
        "Switch Dark-Light Theme") $scriptsDir/theme/DarkLight.sh ;;
